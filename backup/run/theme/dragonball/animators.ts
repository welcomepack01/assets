...
export const DragonBallAnimators: ThemeAnimators = {
    animateObstacle: (obs: Obstacle, elapsedTime: number, delta: number) => {
        // Jump: Scattered Rock Debris (Floating effect)
        if (obs.data.action === 'jump') {
            if (obs.mesh.name === 'RockDebrisGroup') {
                obs.mesh.children.forEach((child) => {
                    if (child.name === 'DebrisPiece') {
                        const ud = child.userData;
                        // Gentle floating (Levitation)
                        child.position.y += Math.sin(elapsedTime * 2.0 + ud.floatOffset) * delta * 0.2;
                        // Slow rotation
                        child.rotation.x += delta * ud.rotSpeedX * 0.5;
                        child.rotation.y += delta * ud.rotSpeedY * 0.5;
                    }
                });
            }
        }
        
        // Crouch: Tunnel (Static)
        
        // Big Jump: Ruined Wall (Static mostly, maybe shake?)
        if (obs.data.action === 'big_jump') {
            // The wall is mostly static, but we could add slight dust particles if we had a system here.
            // For now, static heavy object.
        }
    ...
    animateBoss: (boss: Obstacle, elapsedTime: number, delta: number, currentPhase: number) => {
        if (currentPhase === 1) { // VEGETA
            ...
            if (vegeta) {
                ...
                if (lArm) { lArm.rotation.z = -0.4 + Math.sin(elapsedTime * 20) * 0.05; lArm.rotation.x = -0.1; }
                if (rArm) { rArm.rotation.z = 0.4 - Math.sin(elapsedTime * 20) * 0.05; rArm.rotation.x = -0.1; }
